...
class Bird(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.bird_index = 0
        self.bird_imagem = self.bird_lista[self.bird_index]
        self.bird_imagem_rect = self.bird_imagem.get_rect(center=(3000, 170))

    def desenhar_bird(self, tela):
        bird_size = 1.5
        tela.blit(pygame.transform.smoothscale(self.bird_lista[int(self.bird_index)], (
            (self.bird_lista[int(self.bird_index)]).get_width() / bird_size,
            (self.bird_lista[int(self.bird_index)]).get_height() / bird_size)),
                  self.bird_imagem_rect)
    ...
    def animar_passaro(self):
        ...
        self.bird_imagem_rect.x -= velocidade_corrida * 1.5
        if self.bird_imagem_rect.x < - 40:
            self.bird_imagem_rect.x = randint(2000, 4000)
